...
### Data factory (`tests/test_users.py`)
...
- **Data-driven tests** — the invalid-payload test runs multiple cases (missing email, invalid gender) through one parametrized test function instead of duplicated near-identical tests.
- **Parallel-safe design** — every test builds its own unique data via the factory and doesn't depend on state left behind by other tests.
...
- Extend coverage to GoRest's `posts` and `comments` endpoints
- Move fixtures/factories into dedicated modules if the suite grows beyond a single test file
